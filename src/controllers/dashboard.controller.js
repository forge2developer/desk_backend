...
export class DashboardController {
    ...
    getAdminStats = asyncHandler(async (req, res) => {
        ...
        const data = await dashboardService.getAdminStats(organization);
    ...
    getSalesSummary = asyncHandler(async (req, res) => {
        ...
        const data = await dashboardService.getSalesSummary(organization, startDate, endDate, executiveId);
    ...
    getPreSalesSummary = asyncHandler(async (req, res) => {
        const organization = req.query.organization || req.user?.organization;
        const { startDate, endDate, executiveId } = req.query;
        const data = await dashboardService.getPreSalesSummary(organization, startDate, endDate, executiveId);
    ...
    getMarketingSummary = asyncHandler(async (req, res) => {
        ...
        const data = await dashboardService.getMarketingSummary(organization, startDate, endDate, campaignCategory);
        res.status(200).json(ApiResponse.success('Marketing summary retrieved', data));
    });

    getExecutives = asyncHandler(async (req, res) => {
        const organization = req.query.organization || req.user?.organization;
        const data = await dashboardService.getExecutives(organization);
