...
export class DashboardService {
     ...
     */
    async getAdminStats(organization) {
        ...
        const [
            ...
            Lead.countDocuments({}),
            Lead.countDocuments({ stage: { $in: ['new lead', 'New Lead', 'new_lead', 'New'] } }),
            Lead.countDocuments({ stage: { $in: ['reengaged', 'Reengaged', 're-engaged'] } }),
            Lead.countDocuments({ stage: { $in: ['prospect', 'Prospect', 'active prospect'] } }),
            Lead.countDocuments({ stage: { $in: ['missed followup', 'Missed Followup', 'missed_followup'] } }),
        ]);
        ...
        try {
            ...
            missedCalls = await LeadActivity.countDocuments({
                updates: { $regex: /missed.*call/i }
            });
        } catch (_) {
            missedCalls = 0;
        }
     ...
     */
    async getSalesSummary(organization, startDate, endDate, executiveId) {
        if (!organization) throw new AppError('Organization is required', 400);
        ...
        const baseQuery = {};
        if (executiveId) baseQuery.exe_user = executiveId;
     ...
     * Filtered by date range and optionally by executive
     */
    async getPreSalesSummary(organization, startDate, endDate, executiveId) {
        if (!organization) throw new AppError('Organization is required', 400);
     ...
     */
    async getMarketingSummary(organization, startDate, endDate, campaignCategory) {
        if (!organization) throw new AppError('Organization is required', 400);
